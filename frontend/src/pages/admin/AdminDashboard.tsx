...
import { useAgents, useConversations, useActiveCallsStats } from '@/hooks/use-elevenlabs';
...
export default function AdminDashboard() {
  ...
  const { data: agents = [] } = useAgents();
  const { data: conversations = [] } = useConversations();
  const { data: activeCallsStats } = useActiveCallsStats();
  ...
  return (
    <>
      <div className="space-y-4 md:space-y-6">
        ...
        {/* Stats Grid - 2x3 on mobile, 7 cols on desktop */}
        <div className="grid grid-cols-2 gap-3 md:gap-4 lg:grid-cols-3 xl:grid-cols-7">
          <StatCard
            title="Active Agents"
            value={activeAgents}
            icon={Users}
          ...
          <StatCard
            title="Active Calls"
            value={activeCallsStats?.active_count ?? 0}
            icon={Activity}
