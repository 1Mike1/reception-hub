...
import { useBackendClient } from '@/hooks/use-clients';
...
const ClientDetailPage = () => {
  ...
  // ── Load client from backend ────────────────────────────────────────────────
  const { data: backendClient, isLoading: clientLoading } = useBackendClient(id);
  
  // ── Load ElevenLabs data using agent_id from client ─────────────────────────
  const agentId = backendClient?.agent_id;
  const { data: elAgent, isLoading: agentLoading } = useAgent(agentId);
  ...
  const clientName   = backendClient?.company_name ?? mockClient?.company_name ?? 'Unknown Client';
  const clientEmail  = backendClient?.business_email ?? backendClient?.email ?? mockClient?.business_email ?? '';
  const serviceArea  = backendClient?.service_area ?? mockClient?.service_area ?? '';
  const clientStatus = backendClient ? 'active' : (mockClient?.status ?? 'active');
  const createdAt    = backendClient?.created_at ?? mockClient?.created_at ?? new Date().toISOString();
  ...
  // Not found
  if (!backendClient && !mockClient) {
    return (
      <div className="flex flex-col items-center justify-center py-24 space-y-3">
        <AlertTriangle className="w-12 h-12 text-destructive" />
        <p className="text-lg font-medium">Client Not Found</p>
        <p className="text-sm text-muted-foreground">The requested client could not be found.</p>
