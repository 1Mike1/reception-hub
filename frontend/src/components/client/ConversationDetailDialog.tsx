  ...
  const handleEmailTranscript = async () => {
    ...
    try {
      const res = await fetch(
        `${BACKEND_URL}/conversations/${conv.conversation_id}/send-transcript`,
        { method: 'POST' },
      );
      ...
      toast({
        ...
        description: `Email will be sent to ${body.recipient ?? 'the linked client'}.`,
      });
    } catch (e) {
      toast({
              ...
              <div className="flex items-center justify-between pt-1 pb-2">
                ...
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    <DropdownMenuSeparator />
                    <DropdownMenuItem onClick={handleEmailTranscript} disabled={isSendingEmail}>
                      <Mail className="w-3.5 h-3.5 mr-2" />
                      {isSendingEmail ? 'Sending…' : 'Email transcript'}
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
              </div>
