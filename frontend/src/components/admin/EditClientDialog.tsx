...
export function EditClientDialog({ open, onOpenChange, client }: EditClientDialogProps) {
  ...
  useEffect(() => {
    if (client) {
      const agent = mockAgents.find(a => a.client_id === client.id);
      const planAssignment = mockClientPlanAssignments[client.id];
      
      setFormData({
        company_name: client.company_name,
        business_email: client.business_email,
        service_area: client.service_area,
        assigned_plan_id: planAssignment?.planId || '',
        status: client.status,
        elevenlabs_agent_id: agent?.elevenlabs_agent_id || '',
        linked_phone_numbers: agent?.linked_phone_numbers?.join(', ') || '',
        webhook_processing_enabled: agent?.webhook_enabled ?? true,
        webhook_status: agent?.webhook_enabled ? 'Connected' : 'No Data',
        last_webhook_received: agent?.webhook_enabled ? '2024-12-26T14:30:00Z' : '',
        dynamic_variables: '',
